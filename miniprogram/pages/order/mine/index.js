...
Page({
 data: { items: [], loading: true, busy: false, error: '', more: false, page: 0 },
 ...
 async load(e, reset = false) {
  ...
  try {
   const r = await api.call('order.mine', { page });
   // 还没进服务环节的单才能自己取消，和状态机的转换表保持一致。
   const items = r.items.map(o => ({ ...o, time: api.time(o.createdAt), feeText: o.fee ? (o.fee / 100) + ' 元' : '免跑腿费', canCancel: ['submitted', 'runnerTaken', 'picked'].includes(o.status) }));
 ...
 create() { wx.navigateTo({ url: '/pages/order/create/index' }); }
});

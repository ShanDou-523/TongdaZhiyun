...
Page({
 ...
 async resolveMedia(items) {
  ...
  const urls = {};
  if (ids.length) { try { const r = await wx.cloud.getTempFileURL({ fileList: ids }); r.fileList.forEach(f => { if (f.tempFileURL) urls[f.fileID] = f.tempFileURL; }); } catch (_) { /* 网络异常时回退 fileID 直渲 */ } }

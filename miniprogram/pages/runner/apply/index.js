...
Page({
 data: { runner: null, realName: '', schoolId: '', idCard: '', studentCard: '', devLogin: config.devLogin && !config.mock, loading: true, busy: false, error: '' },
 onShow() { this.load(); },
 async load() {
  try { const me = await api.guard(['customer']); if (!me) return; this.setData({ runner: me.user.runner || null }); }
 ...
 skipPhotos() { this.setData({ idCard: 'dev:idCard', studentCard: 'dev:studentCard', error: '' }); }
});

...
Page({data:{canImportCatalog:config.devLogin,importProgress:'',tab:'users',users:[],stores:[],storeOptions:[],keyword:'',services:[],banners:[],applicants:[],runnerFeeYuan:0,rows:[],tables,tableIndex:0,error:'',loading:false,busy:false,page:0,more:false,edit:null,roleOptions:['客户','门店'],roleIndex:0,storeIndex:0,storeId:'',storeName:'',editingStore:false,storeEnabled:true,serviceId:'',serviceName:'',serviceDesc:'',serviceEnabled:true,editingService:false,categoryOptions:Object.values(categories),categoryKeys:Object.keys(categories),categoryIndex:0,bannerId:'',bannerTitle:'',bannerSubtitle:'',bannerWeight:50,bannerEnabled:true,editingBanner:false,bannerMediaType:'none',bannerMedia:'',versions:['开发版','体验版','正式版'],versionIndex:1,qrStore:0,qr:''},
 ...
 editBanner(e){if(this.data.busy)return;this.bannerRequestId=null;this.bannerGeneration=(this.bannerGeneration||0)+1;const b=this.data.banners.find(x=>x._id===e.currentTarget.dataset.id);this.setData({bannerId:b._id,bannerTitle:b.title,bannerSubtitle:b.subtitle,bannerWeight:b.weight,bannerEnabled:b.enabled,editingBanner:true,bannerMediaType:b.mediaType||'none',bannerMedia:b.mediaFileID||''});},
 newBanner(){if(this.data.busy)return;this.resetBanner();},
 ...
 async saveBanner(){
  ...
  try{
   ...
   this.resetBanner();await this.load(false,true);wx.showToast({title:'已保存'});
  }catch(e){this.setData({error:e.message});}finally{this.setData({busy:false});}
 },
 async removeBanner(e){if(this.data.busy)return;const id=e.currentTarget.dataset.id;const r=await wx.showModal({title:'删除广告',content:'删除后该广告立即从客户端首页消失，且无法恢复。如需暂时隐藏，请改用停用。'});if(!r.confirm)return;this.setData({busy:true,error:''});try{await api.call('admin.bannerDelete',{id});if(this.data.bannerId===id)this.resetBanner();await this.load(false,true);wx.showToast({title:'已删除'});}catch(err){this.setData({error:err.message});}finally{this.setData({busy:false});}},

...
Page({
  ...
  async orders() {
    ...
    try {
      ...
      if (!this.active(generation)) return;
      this.setData({ orders: r.items.map(o => ({ ...o, time: api.time(o.createdAt), feeText: o.fee ? (o.fee / 100) + ' 元' : '', canAccept: (o.status === 'submitted' && ['self','onsite'].includes(o.delivery)) || o.status === 'delivered', canFinish: o.status === 'serving' })) });
    } catch (e) {
      if (this.active(generation)) this.setData({ error: e.message });
    }
  },

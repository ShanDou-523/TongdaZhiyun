...
// 开发期身份切换：登录页填的测试手机号会存成 devActor，每次请求带上，服务端据此判定"这次调用算谁"。
// 前端只决定带不带这个参数，真正的开关在云函数（DEV_LOGIN 环境变量）；生产环境不设，带了也没用。
function devActor() { return config.devLogin ? (wx.getStorageSync('devActor') || '') : ''; }
async function call(action, data = {}) {
  try {
    ...
    let response;
    try { response = await wx.cloud.callFunction({ name: 'api', data: { action, data, token: wx.getStorageSync('session') || '', devActor: devActor() } }); }
